...
const GraficaMontoVendido = () => {
  ...
  useEffect(() => {
    // El flujo de todo esto sería así. Las transacciones son filtradas por .filter() dejando solo las transacciones de tipo Venta, y luego con el metodo .reduce(), reducimos el array a un objeto de objetos que tiene el nombre de la moneda y la cantidad de cada una de ellas.
    ...
    let trnsFiltradasySumadas = Object.values(
      transacciones
        .filter((tran) => tran.tipo_operacion === 2)
        .reduce((agg, trans) => {
  ...
  return (
    <div className="dashboard__item">
      <h2 className="dashboard__item-title">
        Total de $U por monedas vendidas{' '}
      </h2>
        ...
        <Bar
          options={{
            ...
            plugins: {
              ...
              title: {
                display: true,
                text: 'Monto total de monedas vendidas ($U)',
                position: 'top',
              },
            },
          }}
...
export default GraficaMontoVendido;
